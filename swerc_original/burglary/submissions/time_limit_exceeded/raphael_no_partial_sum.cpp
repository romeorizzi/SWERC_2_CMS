// Similar to accepted solution in O(N * max_ladders ^ 4), except that
// we don't use partial sums to compute the scores on each row.
//
// The complexity of this solution is therefore O(N * M *
// max_ladders^4). This should time out.
...
int get_score(int r, const Interval& interval) {
  int s = 0;
  for (int a = interval.i; a <= interval.j; ++a) s += jars[r][a];
  return s;
}
