...
class solLouisGood {
    ...
    static class Auto {
        ...
        int get_def(int s ) {
            for(int l1 = 1 ; l1 < trans[s].length ; l1++)
                for(int l2 = 0 ; l1 < l2 ; l2++)
                    if(trans[s][l1] == trans[s][l2])
                        return trans[s][l1];
            return trans[s][0];
        }
        ...
        int count(int steps) {
            ...
            for(int i = 0 ; i < size ; i++) {
                ...
                for(int l = 0 ; l < alphabet.length() ; l++ )
                    if( base[i] == trans[i][l])
                        nb_base[i]++;
                tr[i] = new int[alphabet.length() - nb_base[i]] ;
                int cur = 0 ;
                for(int l = 0 ; l < alphabet.length() ; l++ )
                    if(trans[i][l] != base[i])
                        tr[i][cur++] = trans[i][l] ;
            ...
            for(int s = 0 ; s < steps ; s++ ) {
                for(int i = 0 ; i < size ; i++) {
                    int res = 0 ;
                    for(int l = 0 ; l < tr[i].length ; l++) {
                        res += c[s&1][tr[i][l]] ;
                    }
                    c[(s+1)&1][i] = (res+c[s&1][base[i]]*nb_base[i])%M ;
                } 
            }
            return c[steps&1][0] ;
        }
